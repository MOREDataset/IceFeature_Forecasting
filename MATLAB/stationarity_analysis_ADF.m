...
for k = 1:height(Vol_FallWinter)
    [h(k),pValue(k),stat(k),cValue(k)] = adftest(Vol_FallWinter(k,1:30),Alpha=0.01);
end
H = mean(h,'omitnan');
...
for k = 1:height(Vol_SpringSummer)
    [h(k),pValue(k),stat(k),cValue(k)] = adftest(Vol_SpringSummer(k,1:30),Alpha=0.01);
end
H = mean(h,'omitnan');
PVALUE = mean(pValue,'omitnan');
STAT = mean(stat,'omitnan');
...
for k = 1:5:height(Vol_FallWinter)
    ...
    temp = EMD(k:k+4,2:31);
    for i = 1:4
        [h(i),pValue(i),stat(i),cValue(i)] = adftest(table2array(temp(i,:)),Alpha=0.01);
    end
    H(j) = mean(h,'omitnan');
