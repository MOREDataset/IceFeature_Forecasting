...
mode = {'ThickTime'}; model = {'TSTPlus','InceptionTimePlus','LSTMPlus'};%};
...
%                         eval(sprintf('%s.%s.%s.%s.Preds_%s.Variables = %s.%s.%s.%s.Preds_%s.Variables*1000;',season{Y},mode{M},Horizon{H},string(strcat('IMF',regexp(IMFS{L},'\d*','match'))),model{k},season{Y},mode{M},Horizon{H},string(strcat('IMF',regexp(IMFS{L},'\d*','match'))),model{k}))
            ...
            if isequal(Y,2)
                path5 =   sprintf('%sThicksequences_%s.csv',Horizon{H},season{Y});
                if ~exist(path5)
            ...
            %             col = Target.Properties.VariableNames;
            %             for j = 2 : length(col)
            %                 for i = 1 :length(Target.(col{j}))
            %                     eval(sprintf('dataTarget.%s(i,:) =  cell2mat(textscan(erase(Target.(col{j}){i},{''['', '']''}),''%%f''));',(col{j})))
            %                 end
            %             end
            %             fld = fieldnames(dataTarget);
            %             tempsum=0;
            %             for i=1:length(fld)
            %                 eval(sprintf('tempsum=tempsum+dataTarget.%s;',fld{i}))
            %             end
...
clear path4 path3 path2 opts model_name model M L k j IMFS i mode Horizon H fld col Y indx O ind season T
        ...
        for H=1:length(Horizon)
            ...
            for i=1:1
                ...
                for k=1:length(model)
                    ...
                    Results.(season{j}).(mode{M}).(Horizon{H}).(IMF_names{i}).(model{k}).RMSE = sqrt(mean(mean((y_test-y_preds).^2)));
                    %         Results.(season{j}).(Horizon{H}).(IMF_names{i}).(model{k}).FS = (1-(Results.(season{j}).(IMF_names{i}).ResNet.RMSE/RMSE_histMean).^2)*100;
                    Results.(season{j}).(mode{M}).(Horizon{H}).(IMF_names{i}).(model{k}).CV = sqrt((1/30)*sum(mean(abs(y_test-y_preds).^2)))/ymean*100;
                    fprintf('  **%s: %.3f,%.3f,%.3f\n',strcat(model{k}),Results.(season{j}).(mode{M}).(Horizon{H}).(IMF_names{i}).(model{k}).RMSE,Results.(season{j}).(mode{M}).(Horizon{H}).(IMF_names{i}).(model{k}).MAPE,Results.(season{j}).(mode{M}).(Horizon{H}).(IMF_names{i}).(model{k}).CV)
                end
            end
        end
...
IMF_names = {'noVMD'};
season={'FallWinter','SpringSummer'};
mode = {'Time', 'ThickTime', 'EMDIMF4', 'IMF31'};
metric = {'RMSE','MAPE','CV'};%,'FS'
model = {'TSTPlus','InceptionTimePlus','LSTMPlus'};
Horizon = {'THICKF1m_'};%'THICKF7d_'
indx = {'31'};
lgds = {'Time','$T$ + Time', '$T^{EMD}$ + Time', '$T^{VMD,31}$ + Time'};
smbls = {'v','^','s','d'};
...
for J = 1:length(IMF_names)
    for k=1:length(season)
        ...
        for H=1:length(Horizon)
            ...
            ylabel('Thickness ($m$)','interpreter','latex','FontSize',size1);
