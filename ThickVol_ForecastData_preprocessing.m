...
% t = tiledlayout(2,1,'TileSpacing','Compact','Padding','tight','units','normalized','outerposition',[0 0 1 1]);
% 
% nexttile
...
Vol_SpringSummer = zeros(length(1 : 1: length(data_temp_SpringSummer.Thickness)-len+1), len);
TS_SpringSummer = NaT(length(1 : 1: length(data_temp_SpringSummer.Thickness)-len+1), len);

for i = 1 : 1: length(data_temp_SpringSummer.Thickness)-len+1
